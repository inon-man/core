...
func setupSimulationApp(tb testing.TB, msg string) (simtypes.Config, dbm.DB, simtestutil.AppOptionsMap, *terraapp.TerraApp) {
	tb.Helper()
	...
	require.Equal(tb, "TerraApp", app.Name())
	return config, db, appOptions, app
}

func fullAppSimulation(tb testing.TB) {
	tb.Helper()
	config, db, _, app := setupSimulationApp(tb, "skipping application simulation")
...
func BenchmarkFullAppSimulation(b *testing.B) {
	...
	simcli.FlagNumBlocksValue = 1000
	simcli.FlagBlockSizeValue = 200
...
func TestFullAppSimulation(t *testing.T) {
	...
	simcli.FlagEnabledValue = true
	simcli.FlagNumBlocksValue = 200
	simcli.FlagBlockSizeValue = 25
	simcli.FlagCommitValue = true
	simcli.FlagVerboseValue = true
	simcli.FlagPeriodValue = 10
	simcli.FlagSeedValue = 11
	fullAppSimulation(t)
}
