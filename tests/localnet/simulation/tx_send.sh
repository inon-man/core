...
while true; do
    ...
    balance1=$(terrad q bank balances $addr1 -o json | jq -r '.balances | if length > 0 then .[] | select(.denom == "uluna").amount else "0" end')
    if [ $balance1 -lt 500000000 ]; then
        continue
    fi

    terrad tx bank send $addr1 $addr2 1000000uluna --chain-id $CHAIN_ID --home $NODE_HOME --gas auto --gas-adjustment 2.3 --fees 20000000uluna --keyring-backend $KEYRING_BACKEND -y
    if [ $? -eq 0 ]; then
        success=$((success+1))
    fi
    sleep 10
