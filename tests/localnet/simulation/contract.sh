...
for j in $(seq 0 1); do
	...
	id=$(terrad q tx $txhash -o json | jq -r '.raw_log' | jq -r '.[0].events[1].attributes[1].value')
	...
	sleep 10
	txhash=$(echo $out | jq -r '.txhash')
	contract_addr=$(terrad q tx $txhash -o json | jq -r '.raw_log' | jq -r '.[0].events[0].attributes[3].value')
	...
	for i in $(seq 0 3); do
		...
		out=$(terrad tx wasm execute $contract_addr "$msg" --from test$j --output json --gas auto --gas-adjustment 2.3 --fees 20000000uluna --chain-id $CHAIN_ID --home $NODE_HOME --keyring-backend $KEYRING_BACKEND -y))
		code=$(echo $out | jq -r '.code')
		if [ "$code" != "0" ]; then
