...
test-sim-determinism:
	...
	-Enabled=true -NumBlocks=50 -BlockSize=200 -Commit=true -Period=0 -v -timeout 24h
...
test-e2e-ci:
	@VERSION=$(VERSION) TERRA_E2E=True TERRA_E2E_DEBUG_LOG=False TERRA_E2E_UPGRADE_VERSION=$(E2E_UPGRADE_VERSION) go test -mod=readonly -timeout=25m -v $(PACKAGES_E2E) -p 4

test-e2e-ci-no-cleanup:
	@VERSION=$(VERSION) TERRA_E2E=True TERRA_E2E_DEBUG_LOG=False TERRA_E2E_SKIP_CLEANUP=True TERRA_E2E_UPGRADE_VERSION=$(E2E_UPGRADE_VERSION) go test -mod=readonly -timeout=25m -v $(PACKAGES_E2E) -p 4

# test-e2e-ci-scheduled runs every e2e test available, and is only run on a scheduled basis
test-e2e-ci-scheduled:
	@VERSION=$(VERSION) TERRA_E2E_SCHEDULED=True TERRA_E2E=True TERRA_E2E_DEBUG_LOG=False TERRA_E2E_UPGRADE_VERSION=$(E2E_UPGRADE_VERSION) go test -mod=readonly -timeout=25m -v $(PACKAGES_E2E) -p 4
...
test-e2e-debug: e2e-setup
	@VERSION=$(VERSION) TERRA_E2E=True TERRA_E2E_DEBUG_LOG=True TERRA_E2E_UPGRADE_VERSION=$(E2E_UPGRADE_VERSION) go test -mod=readonly -timeout=25m -v $(PACKAGES_E2E) -count=1
...
test-e2e-short: e2e-setup
	@VERSION=$(VERSION) TERRA_E2E=True TERRA_E2E_DEBUG_LOG=True TERRA_E2E_SKIP_UPGRADE=True TERRA_E2E_SKIP_IBC=True TERRA_E2E_SKIP_STATE_SYNC=True TERRA_E2E_SKIP_CLEANUP=True go test -mod=readonly -timeout=25m -v $(PACKAGES_E2E) -count=1
