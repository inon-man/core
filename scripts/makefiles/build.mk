...
build-dev-install: go.sum
	GOWORK=off go install $(DEBUG_BUILD_FLAGS) $(GC_FLAGS) $(GO_MODULE)/cmd/terrad
...
build-install-with-autocomplete: build-check-version go.sum
	...
	if echo "$$PARENT_SHELL" | grep -q "zsh"; then \
		if ! grep -q ". <(terrad enable-cli-autocomplete zsh)" ~/.zshrc; then \
			echo ". <(terrad enable-cli-autocomplete zsh)" >> ~/.zshrc; \
			echo; \
		...
		if ! grep -q -e "\. <(terrad enable-cli-autocomplete bash)" -e '\[\[ -r "/opt/homebrew/etc/profile.d/bash_completion.sh" \]\] && \. "/opt/homebrew/etc/profile.d/bash_completion.sh"' ~/.bash_profile; then \
			brew install bash-completion; \
			echo '[ -r "/opt/homebrew/etc/profile.d/bash_completion.sh" ] && . "/opt/homebrew/etc/profile.d/bash_completion.sh"' >> ~/.bash_profile; \
			echo ". <(terrad enable-cli-autocomplete bash)" >> ~/.bash_profile; \
			echo; \
			echo; \
		...
		if ! grep -q ". <(terrad enable-cli-autocomplete bash)" ~/.bash_profile; then \
...
build-reproducible-amd64: go.sum
	mkdir -p $(BUILDDIR)
	$(DOCKER) buildx create --name terrabuilder || true
	$(DOCKER) buildx use terrabuilder
	$(DOCKER) buildx build \
		...
		--load \
		-f Dockerfile .
	$(DOCKER) rm -f terrabinary || true
	$(DOCKER) create -ti --name terrabinary terra:local-amd64
	$(DOCKER) cp terrabinary:/bin/terrad $(BUILDDIR)/terrad-linux-amd64
	$(DOCKER) rm -f terrabinary
...
build-reproducible-arm64: go.sum
	...
	$(DOCKER) buildx build \
		...
		-f Dockerfile .
	$(DOCKER) rm -f terrabinary || true
	$(DOCKER) create -ti --name terrabinary terra:local-arm64
	$(DOCKER) cp terrabinary:/bin/terrad $(BUILDDIR)/terrad-linux-arm64
	$(DOCKER) rm -f terrabinary

build-linux-goos: go.sum
